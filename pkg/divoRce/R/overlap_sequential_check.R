...
#'
#' Intended for data where one suspects overlap, this function checks for overlap on an ever growing subset of the data. It is a mid-level function for a response vector y and a design matrix X. 
#'
#' The function samples a fraction of observations from the data and checks for overlap. If overlap exists in a subset, then overlap exists in the overal data (Sablica et al,. 2026). If no overlap is found, it takes a larger sample and checks again until all data are tested. If no overlap is found even for all data, it concludes there is separation.
#'
#' Since solving the exact linear program on the full data via \link{check_overlap} can take a long time for large data, this check can be quicker in case of overlap (especially if the overlapping categories are not rare). However, if there is separation this function usually takes longer. 
#'
#' @param y outcome vector. 
#' @param X design matrix.
#' @param S structure vector matrix 
#' @param frac the fraction of the data to use for sequential checking (uses n/frac data). Defaults to 10. If frac is below 1 or n, it uses frac=1. Using frac=1 is the same as using check_overlap.  
...
#'
#' @export
overlap_sequential_check <- function(y, X, S, frac=10L, verbose=FALSE, rational=FALSE, model=c("b","bcl","cl","acl","sl","os"), backend = c("rcdd", "ROI"), solver = NULL)
{
 ...
 {
     olcheck <- check_overlap(y=ys, X=Xs, rational=rational, model=model, backend=backend, solver = solver)
 }
 ...
 ind <- sample(1:n,nc,replace=FALSE)
 Ss <- S[ind,]
 olcheck <- check_overlap(S=Ss, rational=rational, backend = backend, solver = solver)
 if(isTRUE(olcheck)) break()
 if(nco >= n) break() 
 i <- i+1
 }
